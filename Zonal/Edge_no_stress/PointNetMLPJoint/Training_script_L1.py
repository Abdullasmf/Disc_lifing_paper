...
def main(preset_name: str = "S0", batch=8) -> None:
    ...
    # # [fix_ablation_extra_feat_cols] patched: hardcoded for Edge_no_stress ablation
    EXTRA_FEAT_COLS = []
    if width0 < 4:
        raise RuntimeError(
            f"Dataset width {width0} is too narrow for Edge_no_stress ablation "
            f"(need at least 4 columns). Wrong H5 file?"
        )
    print(f"EXTRA_FEAT_COLS=[] (0 extra feature(s)) for Edge_no_stress ablation")
